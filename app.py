...
# # template = st.text_input("Pregunta", value="", max_chars=1000, key="question")
# template = """
...
# question_file = st.file_uploader("Archivo de pregunta", type=["txt"], key="question_file")
...
#     wrapped_text = textwrap.fill(response["resumen"], width=100, break_long_words=False, replace_whitespace=False)
...
template = """
Question: resumir el siguiente texto: {question}
Answer: resumen.
"""

prompt = PromptTemplate(template=template, input_variables=["question"])
llm_chain = LLMChain(prompt=prompt, llm=llm)
# st.title("Resumen del texto")
# question = st.file_uploader("Resumen")
# if question is not None:
#     content = question.read().decode("utf-8")
#     st.write(content)
content = """  
